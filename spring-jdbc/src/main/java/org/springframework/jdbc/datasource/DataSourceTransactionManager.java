...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	...
	protected Object doGetTransaction() {
		...
		// 默认设置的 newConnectionHolder 为 false
	...
	protected boolean isExistingTransaction(Object transaction) {
		...
		// 判断 connectionHolder != null，并且里面的 transactionActive 属性是否为 true
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			if (!txObject.hasConnectionHolder() ||
				...
				// 将新获取的 connection 设置到 connectHolder，同时设置 newConnectHolder 为 true
			...
			// 从 ConnectionHolder 里面获取 Connection
			con = txObject.getConnectionHolder().getConnection();

			// 设置 readonly 和 isolation
			// 这里有两个特殊的地方：
			// 		1. 如果设置了 readonly = true，里面会把 connection 的 isolation 设置为 true
			// 		2. 如果设置了 isolation 不是 DEFAULT，并且定义的 isolation 和 connection 的 isolation 不一致的话
			// 			会将 connection isolation 返回，同时设置 connection 的 isolation 为指定的 isolation
			...
			// 如果是 readonly 的事务，这里需要通过 connection 执行 SQL 语句：SET TRANSACTION READ ONLY
			...
			// 如果 newConnectionHolder 这个值是 true，代表是新建的事务，这里需要绑定到当前线程
			...
			// 如果出现异常，并且是新建 Connection，这里需要进行释放 Connection
			if (txObject.isNewConnectionHolder()) {
