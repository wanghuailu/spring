...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		// 将 TransactionManager 转化为 PlatformTransactionManager
		PlatformTransactionManager ptm = asPlatformTransactionManager(tm);
		// 获取连接点的唯一标识  类名 + 方法名
		final String joinpointIdentification = methodIdentification(method, targetClass, txAttr);

		// DataSourceTransactionManager 并没有实现 CallbackPreferringPlatformTransactionManager
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		// 如果没有指定名称，那么使用 类名 + 方法名 来标识，并用 DelegatingTransactionAttribute 来封装 txAttr
	...
	protected void commitTransactionAfterReturning(@Nullable TransactionInfo txInfo) {
		// 事务的 transactionStatus 不能为 null，正常情况下也不会为 null
	...
	protected void completeTransactionAfterThrowing(@Nullable TransactionInfo txInfo, Throwable ex) {
		if (txInfo != null && txInfo.getTransactionStatus() != null) {
			...
			// 判断配置的 rollback-for 配置
			// 默认是回滚 RuntimeException 和 Error
			// 这里需要去看 DelegatingTransactionAttribute 和 RuleBasedTransactionAttribute 这两个子类的实现逻辑
