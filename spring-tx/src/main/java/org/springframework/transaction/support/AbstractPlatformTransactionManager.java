...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		// 开启事务，这里只是创建了一个 DataSourceTransactionObject，设置了基本属性
		// 设置了 newConnectionHolder、mustRestoreAutoCommit、readOnly、savepointAllowed 等属性
		Object transaction = doGetTransaction();
		boolean debugEnabled = logger.isDebugEnabled();

		// 如果已经存在事务，则需要判断当前事务的传播机制
		// 如果是第一次创建事务，这里不会进去
		...
		// 判断定义的事务超时时间是否小于默认值 -1，事务超时时间配置，必须要大于等于 -1
		if (def.getTimeout() < TransactionDefinition.TIMEOUT_DEFAULT) {
			throw new InvalidTimeoutException("Invalid transaction timeout", def.getTimeout());
		}

		// No existing transaction found -> check propagation behavior to find out how to proceed.
		// PROPAGATION_MANDATORY 必须存在一个事务，如果不存在，则抛出异常；前面已经判断是否已经存在事务，所以执行到这里，一定是没有事务的
		...
		// 		如果当前存在事务，则加入该事务，如果不存在，则创建一个新的事务，这个是默认的传播机制

		// PROPAGATION_REQUIRES_NEW
		// 		如果当前存在事务，先把当前事务相关内容封装到一个实体，然后重新创建一个新事务，接受这个实体为参数，用于事务的恢复。
		// 		更直白的说法就是暂停当前事务(当前无事务则不需要)，创建一个新事务。
		// 		针对这种情况，两个事务没有依赖关系，可以实现新事务回滚了，但外部事务继续执行

		// PROPAGATION_NESTED：
		// 		如果当前存在事务，则使用 SavePoint 技术把当前事务状态进行保存，然后底层共用一个连接
		// 		当NESTED内部出错的时候，自行回滚到 SavePoint这个状态
		// 		只要外部捕获到了异常，就可以继续进行外部的事务提交，而不会受到内嵌业务的干扰，
		// 		但是，如果外部事务抛出了异常，整个大事务都会回滚

		// 		注意：前面已经判断是否已经存在事务，所以执行到这里，一定是没有事务的，这里就是直接创建一个新的事务
