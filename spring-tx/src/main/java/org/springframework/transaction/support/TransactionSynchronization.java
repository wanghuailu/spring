...
public interface TransactionSynchronization extends Ordered, Flushable {
	 ...
	 *
	 * 事务提交后可以执行一些逻辑操作
	 * 注意：这个时候事务已经提交了，如果在 afterCommit 里面再进行数据库的操作可能不会被提交（with no commit following anymore！）
	 * 		mybatis会提交，JPA不会进行提交（Hibernate）
	 * 如果一定需要进行数据操作，最好的操作还是使用 PROPAGATION_REQUIRES_NEW 来保证
	 */
